...
# ============================================================================
# MolniyaOS — set the CPU scaling governor on every core
# ============================================================================
# Installed as /usr/local/sbin/molniya-set-governor by install-governor.sh, and
# called by molniya-governor.service at boot. Also usable by hand:
#
#   sudo molniya-set-governor performance
#   sudo molniya-set-governor ondemand      # to put it back for a comparison
#
...
if [ ! -e "${GOV_GLOB[0]}" ]; then
    echo "molniya-set-governor: no cpufreq interface on this system" >&2
    exit 1
fi
...
if [ -r "$AVAIL_FILE" ] && ! grep -qw -- "$WANT" "$AVAIL_FILE"; then
    echo "molniya-set-governor: '$WANT' is not an available governor" >&2
...
for path in "${GOV_GLOB[@]}"; do
    ...
    if echo "$WANT" > "$path" 2>/dev/null; then
        ...
        echo "molniya-set-governor: could not write $path" >&2
    fi
done

if [ "$written" -eq 0 ]; then
    echo "molniya-set-governor: failed to set '$WANT' on any core" >&2
    exit 1
fi
...
if [ "$actual" != "$WANT" ]; then
    echo "molniya-set-governor: wrote '$WANT' but cpu0 reads '$actual'" >&2
    exit 1
fi

echo "molniya-set-governor: '$WANT' set on $written core(s)"
