...
# ============================================================================
# MolniyaOS — build the root filesystem in a chroot (Phase 4a, stage 2)
# ============================================================================
# Takes the pinned base image from stage 1 and turns it into a MolniyaOS rootfs.
...
#   ./build-rootfs.sh --kernel <tarball>  also install MolniyaOS kernel modules
...
neutralise_firstboot() {
    ...
    # on, and disabling it would be MolniyaOS overriding a platform convention for
    # no gain.
...
chroot_prepare() {
    ...
    sudo cp -a "$ROOTFS/etc/resolv.conf" "$ROOTFS/etc/resolv.conf.molniya-bak" 2>/dev/null || true
...
chroot_finish() {
    ...
    if [ -e "$ROOTFS/etc/resolv.conf.molniya-bak" ]; then
        sudo mv "$ROOTFS/etc/resolv.conf.molniya-bak" "$ROOTFS/etc/resolv.conf"
    fi
}
...
install_kernel_modules() {
    ...
    step "installing MolniyaOS kernel modules from $(basename "$tarball")"
...
write_manifest() {
    ...
    {
        echo "# MolniyaOS rootfs manifest — generated by image/build-rootfs.sh"
        echo "base_image     $(basename "$img")"
        echo "base_sha256    $("$SELF_DIR/fetch-base.sh" --print-pin | awk '/^sha256/{print $2}')"
        echo "kernel         ${kernel:-NONE — rootfs has no MolniyaOS kernel modules}"
...
main() {
    ...
    [ -n "$kver" ] || note "WARNING: no MolniyaOS kernel — this is not yet a MolniyaOS image"
