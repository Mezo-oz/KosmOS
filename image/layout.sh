...
# ============================================================================
# MolniyaOS — A/B partition layout: the single source of truth
# ============================================================================
...
#   ./layout.sh slotmap                > /etc/molniya/slots.conf
#   ./layout.sh summary
#   ./layout.sh min-bytes
# ============================================================================
...
# MolniyaOS image; the assembled image built earlier that day only fit because the
...
readonly LABEL_DATA="MOLNIYADATA"
...
readonly RAUC_COMPATIBLE="molniya-rpi5"
...
emit_sfdisk() {
    cat <<-EOF
	# MolniyaOS A/B partition table — generated by image/layout.sh, do not hand-edit.
...
emit_rauc() {
    ...
    cat <<-EOF
	# MolniyaOS RAUC slot definitions — generated by image/layout.sh, do not hand-edit.
	[system]
	...
	[keyring]
	path=/etc/rauc/molniya.cert.pem
...
# slots -- that is the entire point of it. Which MolniyaOS state actually lives
...
emit_fstab() {
    ...
    echo "# MolniyaOS slot $slot — generated by image/layout.sh. Do not hand-edit."
...
#
# The image build writes this to /etc/molniya/slots.conf. Deliberately dumb
...
emit_slotmap() {
    echo "# MolniyaOS slot map — generated by image/layout.sh. Do not edit."
    echo "# Consumed by image/health-check/slot-identity.sh."
    echo "MOLNIYA_SLOT_A_BOOT=${PN_BOOT_A}"
    echo "MOLNIYA_SLOT_A_ROOT=${PN_ROOT_A}"
    echo "MOLNIYA_SLOT_B_BOOT=${PN_BOOT_B}"
    echo "MOLNIYA_SLOT_B_ROOT=${PN_ROOT_B}"
    echo "MOLNIYA_SELECTOR_PARTITION=${PN_AUTOBOOT}"
    echo "MOLNIYA_DATA_PARTITION=${PN_DATA}"
    echo "MOLNIYA_COMPATIBLE=${RAUC_COMPATIBLE}"
}
...
usage() {
	...
	usage: layout.sh <command> [args]
	  ...
	  slotmap                   /etc/molniya/slots.conf for the target
